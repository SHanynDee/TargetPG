...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  G4double Ep = prestep -> GetKineticEnergy();

  if(endPoint->GetProcessDefinedStep() -> GetProcessName() == "protonInelastic") //for protoninelastic you will have a gamma produced (gamma gate)
  {
   ...
   // collect energy deposited in this step
    ...
    for (size_t lp=0; lp<(*secondary).size(); lp++)
    {
      ...
      if(name == "gamma") gammaEkin.push_back(energy); //if you have a gamma then you fill gamma kinetic energy
      if(type == "nucleus") 
      {
...
//note that postPoint->GetStepStatus() == fGeomBoundary means that the particle is leaving the current volume
    ...
    for(int i = 0; i<gammaEkin.size(); i++) //root file only filled when you have a gamma energy
      ...
      analysis -> FillVertex(gammaEkin[i], FinalA, FinalZ, protonStep, protonE, edepStep, position.getX()/mm, position.getY()/mm, position.getZ()/mm, dose, rmsDose);
